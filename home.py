...
st.write("Below you can enter the descriptors of a potential game and a graphic will automatically appear")

st.write("If you want to see the prediction for a game, click the 'Try out the Predictor?' button and your input will be fed into the model. If you want a prediction without any descriptors, just hit the 'Try out the Predictor?' button ")
...
if (clicked):
    if user_descriptors == []:
        user_descriptors == ['no_descriptors']
        count = len(user_descriptors) 
        new_game_values = [] 
        for descriptor in descriptor_list: 
            if (descriptor in user_descriptors):
